export default class VesiHexSightPolygon extends PointSourcePolygon {
  /**
   ...
   */
  _compute() {
    const hex = vesi.region.getHexFromPoint(this.origin);
    this.points = VesiHexSightPolygon.getHexVisibilityPoints(hex);
  }
  ...
  static getHexVisibilityPoints(hex) {
    ...
    if (game.release.generation >= 12) ({ sizeX, sizeY } = hex.grid)
    else ({ w: sizeX, h: sizeY } = canvas.grid.grid);
    const poly = [];

    // Check world settings to see which exploration mode they chose
    const setting = game.settings.get("pf2e-vesi-tools", "fogExplorationRadius") || "default";
